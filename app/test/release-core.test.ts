...
function classification(tier: RiskTier, s: Partial<ClassificationSignals> = {}, d: RegulatoryDomain[] = ['NONE_IDENTIFIED']) {
  const sig = signals(s);
  return { classification_version: 1, risk_tier: tier, risk_floor: tier, ai_proposed_tier: tier, classification_method: 'HUMAN_CONFIRMED' as const, regulatory_domains: d, data_classes: sig.data_classes, integration_types: sig.integration_types, ai_behavior: sig.ai_behavior, autonomy_level: sig.autonomy_level, deployment_surface: sig.deployment_surface, rationale: 'x', floor_reasons: [], confidence: 0.9, confirmed_by: 'u', confirmed_at: 'x', classifier_version: 'v' };
}
function profileFor(tier: RiskTier, s: Partial<ClassificationSignals> = {}, d: RegulatoryDomain[] = ['NONE_IDENTIFIED']) {
  ...
  p.policy_profile_id = 'pp-1'; p.policy_profile_hash = sha256('policy-' + tier); p.generated_at = 'x';
  return p;
}
function planFor(tier: RiskTier, s: Partial<ClassificationSignals> = {}, d: RegulatoryDomain[] = ['NONE_IDENTIFIED']) {
  const plan = compileEnforcementPlan({ profile: profileFor(tier, s, d), classification: classification(tier, s, d), enforcement_plan_version: 1 });
  plan.enforcement_plan_id = 'ep-1'; plan.enforcement_plan_hash = sha256('plan-' + tier); plan.generated_at = 'x';
  return plan;
}
...
function rc(over: Partial<ReleaseCandidate> = {}): Omit<ReleaseCandidate, 'release_candidate_id' | 'created_at' | 'created_by' | 'release_candidate_hash' | 'status' | 'supersedes_release_candidate_id'> {
  return {
    qhub_app_id: 'app-1', qhub_app_version: 1, conversation_id: 'conv-1',
    source_commit: null, canonical_file_manifest_hash: sha256(canonicalFileManifestString(FILES)), file_count: FILES.length,
    build_artifact_digest: null, dependency_lockfile_hash: sha256('lock-v1'),
    classification_version: 1, classification_reference: 'chain-1', risk_tier: 'T2',
    policy_profile_id: 'pp-1', policy_profile_version: 1, policy_profile_hash: sha256('policy-T2'),
    enforcement_plan_id: 'ep-1', enforcement_plan_version: 1, enforcement_plan_hash: sha256('plan-T2'),
    model_manifest_hash: sha256('models:claude-sonnet-4-6'), connector_manifest_hash: sha256('connectors:sor'), data_access_manifest_hash: sha256('data:client_pii'),
    target_environment: 'PRODUCTION', deployment_target: 'netlify:prod', release_scope: 'full',
...
describe('deriveAttestationRequirements (from policy, not tier alone)', () => {
  it('T0 requires no formal attestation', () => {
    expect(deriveAttestationRequirements(profileFor('T0', { data_classes: ['PUBLIC'] }), planFor('T0', { data_classes: ['PUBLIC'] }))).toHaveLength(0);
  });
  it('T2 requires business-owner attestation', () => {
    const reqs = deriveAttestationRequirements(profileFor('T2', { data_classes: ['CLIENT_PII'], integration_types: ['EXTERNAL_SYSTEM_OF_RECORD'] }, ['BOOKS_AND_RECORDS']), planFor('T2', { data_classes: ['CLIENT_PII'], integration_types: ['EXTERNAL_SYSTEM_OF_RECORD'] }, ['BOOKS_AND_RECORDS']));
    expect(reqs.map((r) => r.purpose)).toContain('BUSINESS_OWNER');
  });
  it('T3 requires owner + governance (dual, 2 distinct) + change/technology', () => {
    const s = { data_classes: ['MNPI' as const], integration_types: ['TRADING_OR_ORDERS' as const], autonomy_level: 'AUTONOMOUS' as const, deployment_surface: 'PRODUCTION' as const };
    const reqs = deriveAttestationRequirements(profileFor('T3', s, ['SEC', 'FINRA', 'CFTC']), planFor('T3', s, ['SEC', 'FINRA', 'CFTC']));
...
describe('statement + receipt hashes', () => {
  it('statement hash stable for identical inputs', () => {
    const p = { version: 'v1', purpose: 'BUSINESS_OWNER' as const, statement: 'I attest...', release_candidate_hash: 'abc', target_environment: 'PRODUCTION', scope: 'full' };
  ...
  it('receipt hash stable + changes with release hash', () => {
    const base = { receipt_version: 'v1', release_candidate_id: 'rc-1', release_candidate_hash: 'H1', qhub_app_id: 'app-1', qhub_app_version: 1, target_environment: 'PRODUCTION' as const, risk_tier: 'T2' as const, classification_version: 1, policy_profile_hash: 'p', enforcement_plan_hash: 'e', required_attestation_purposes: ['BUSINESS_OWNER' as const], completed_attestations: [], deployment_decision: 'APPROVE' as const, deployment_decision_id: 'd1', known_exceptions: [] };
    expect(sha256(canonicalReceiptString(base))).toBe(sha256(canonicalReceiptString({ ...base })));
    expect(sha256(canonicalReceiptString(base))).not.toBe(sha256(canonicalReceiptString({ ...base, release_candidate_hash: 'H2' })));
  });
});
