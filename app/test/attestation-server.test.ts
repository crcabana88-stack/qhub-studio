...
const H = vi.hoisted(() => ({
  ...
  getReleaseCandidateCount: vi.fn(),
  upsertReleaseCandidate: vi.fn(),
  freezeRC: vi.fn(),
  supersede: vi.fn(),
  setStatus: vi.fn(),
  insertAttestation: vi.fn(),
  getAttestationsForRelease: vi.fn(),
...
vi.mock('~/lib/qhub/attestation-store.server', () => ({
  getReleaseCandidate: H.getReleaseCandidate,
  getReleaseCandidateCount: H.getReleaseCandidateCount,
  upsertReleaseCandidate: H.upsertReleaseCandidate,
  freezeReleaseCandidate: H.freezeRC,
  supersedeOtherReleases: H.supersede,
  setReleaseStatus: H.setStatus,
  insertAttestation: H.insertAttestation,
  getAttestationsForRelease: H.getAttestationsForRelease,
  revokeAttestation: H.revokeAttestation,
  recordDeploymentDecision: H.recordDecision,
}));
vi.mock('~/lib/qhub/governance-service.server', () => ({
  createGovernanceService: () => ({ recordReleaseEvent: H.recordReleaseEvent }),
}));
...
function profileFor(tier: RiskTier, s: Partial<ClassificationSignals> = {}, d: any[] = ['NONE_IDENTIFIED']) {
  ...
  p.policy_profile_hash = sha256(canonicalPolicyString(p));
  p.generated_at = 'x';
...
function planFor(tier: RiskTier, s: Partial<ClassificationSignals> = {}, d: any[] = ['NONE_IDENTIFIED']) {
  const plan = compileEnforcementPlan({
    ...
    classification: {
      classification_version: 1,
      risk_tier: tier,
      data_classes: signals(s).data_classes,
      integration_types: signals(s).integration_types,
      ai_behavior: signals(s).ai_behavior,
      autonomy_level: signals(s).autonomy_level,
      deployment_surface: signals(s).deployment_surface,
      regulatory_domains: d,
    } as any,
...
function rcRow(tier: RiskTier, over: any = {}) {
  const p = profileFor(
    tier,
    tier === 'T3' ? T3S : { data_classes: ['CLIENT_PII'], integration_types: ['EXTERNAL_SYSTEM_OF_RECORD'] },
    tier === 'T3' ? T3D : ['BOOKS_AND_RECORDS'],
  );
  const plan = planFor(
    tier,
    tier === 'T3' ? T3S : { data_classes: ['CLIENT_PII'], integration_types: ['EXTERNAL_SYSTEM_OF_RECORD'] },
...
function att(purpose: string, role: string, signer: string, over: any = {}) {
  return {
    attestation_id: 'a-' + signer + purpose,
...
async function evaluate(over: any = {}) {
  ...
  return evaluateReleaseForDeployment({
...
async function sign(over: any = {}) {
  ...
  return signAttestation({
    ...
    conversationId: 'conv-1',
    release_candidate_id: 'rc-1',
    attestation_purpose: 'BUSINESS_OWNER',
    sessionId: 's',
    env: ENV,
    ...over,
  });
...
beforeEach(() => {
  ...
  H.freezeRC.mockResolvedValue(true);
  H.supersede.mockResolvedValue(undefined);
  H.setStatus.mockResolvedValue(undefined);
  H.insertAttestation.mockResolvedValue({ ok: true });
  H.recordDecision.mockResolvedValue(true);
...
describe('evaluate for deployment (tests 18-22)', () => {
  ...
  it('attestation for a DIFFERENT release hash cannot approve (test 18)', async () => {
    H.getReleaseCandidate.mockResolvedValue(rcRow('T2'));
    H.getAttestationsForRelease.mockResolvedValue([
      att('BUSINESS_OWNER', 'owner', 'owner-1', { release_candidate_hash: 'OTHER' }),
    ]);
  ...
  it('expired/revoked attestation is ignored (tests 15/16)', async () => {
    H.getReleaseCandidate.mockResolvedValue(rcRow('T2'));
    H.getAttestationsForRelease.mockResolvedValue([
      att('BUSINESS_OWNER', 'owner', 'owner-1', { status: 'EXPIRED' }),
      att('BUSINESS_OWNER', 'owner', 'owner-2', { status: 'REVOKED' }),
    ]);
    expect((await evaluate()).decision).toBe('REJECT');
  });
  it('T3 needs owner + two DISTINCT governance signers (tests 13/14/20)', async () => {
    ...
    H.getAttestationsForRelease.mockResolvedValue([
      att('BUSINESS_OWNER', 'owner', 'owner-1'),
      att('GOVERNANCE', 'governance', 'gov-1'),
    ]);
    expect((await evaluate()).decision).toBe('REJECT');
    ...
    H.getAttestationsForRelease.mockResolvedValue([
      att('BUSINESS_OWNER', 'owner', 'owner-1'),
      att('GOVERNANCE', 'governance', 'gov-1'),
      att('GOVERNANCE', 'governance', 'gov-1', { attestation_id: 'dup' }),
    ]);
    expect((await evaluate()).decision).toBe('REJECT');
    ...
    H.getAttestationsForRelease.mockResolvedValue([
      att('BUSINESS_OWNER', 'owner', 'owner-1'),
      att('GOVERNANCE', 'governance', 'gov-1'),
      att('GOVERNANCE', 'compliance', 'gov-2'),
      att('TECHNOLOGY', 'owner', 'owner-1'),
    ]);
...
describe('staleness & environment invalidation (tests 6/7/8)', () => {
  it('policy revision invalidates (POLICY_STALE)', async () => {
    ...
    H.getPolicyProfile.mockResolvedValue({
      ...(await import('~/lib/qhub/policy-engine')),
      policy_profile_hash: 'CHANGED',
    } as any);
  ...
  it('enforcement-plan drift after freeze invalidates (PLAN_STALE)', async () => {
    // Classification and policy still match, but the active plan has drifted from the frozen plan hash.
    H.getReleaseCandidate.mockResolvedValue(rcRow('T2'));
    H.getActivePlan.mockResolvedValue({ plan: { enforcement_plan_hash: 'DRIFTED' } });

    const r = await evaluate();
    expect(r.decision).toBe('REJECT');
    expect(r.reason_codes).toContain('PLAN_STALE');
  });
