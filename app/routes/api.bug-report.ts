...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- QHUB Studio: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via QHUB Studio bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'quantex-tech/qhub-studio';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'QHUB Studio-bug-reporter',
    });
