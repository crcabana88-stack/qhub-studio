...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  switch (body.op) {
    ...
    case 'receipt': {
      ...
      const atts = (await getAttestationsForRelease(rc.release_candidate_id, session.orgId, env)).filter(
        (a) => a.status === 'VALID',
      );
      ...
      const receiptBase = {
        ...
        completed_attestations: atts.map((a) => ({
          attestation_id: a.attestation_id,
          purpose: a.attestation_purpose,
          signer_role: a.signer_role,
        })),
        deployment_decision:
          rc.status === 'APPROVED' ? ('APPROVE' as const) : rc.status === 'REJECTED' ? ('REJECT' as const) : null,
        deployment_decision_id: null,
        known_exceptions: [] as string[],
      };
      const receiptHash = sha256(canonicalReceiptString(receiptBase));

      return json({
        ok: true,
        receipt: { ...receiptBase, generated_at: new Date().toISOString(), receipt_hash: receiptHash },
      });
    }
