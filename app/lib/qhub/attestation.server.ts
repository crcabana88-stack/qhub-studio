...
interface Session { userId: string; orgId: string; role: string; }
...
export interface FreezeResult {
  ...
  required_attestations?: { purpose: AttestationPurpose; roles: string[]; min_signers: number; distinct_signers: boolean }[];
  status?: string;
}
...
export async function freezeReleaseCandidate(input: FreezeInput): Promise<FreezeResult> {
  ...
  const app = await getOrCreateQhubApp({ orgId: session.orgId, userId: session.userId, conversationId }, env).catch(() => null);

  if (!app) return { ok: false, reason: 'RELEASE_NOT_FOUND' };
  if (app.org_id !== session.orgId) return { ok: false, reason: 'TENANT_MISMATCH' };

  const classification = await getClassification(app.qhub_app_id, env);
  if (!classification) return { ok: false, reason: 'CLASSIFICATION_STALE' };
  ...
  const release_candidate_hash = sha256(canonicalReleaseCandidateString(partial));
  ...
  if (!up.ok || !up.rc) return { ok: false, reason: 'RELEASE_NOT_FOUND' };
  ...
  const gov = createGovernanceService({ userId: session.userId, orgId: session.orgId, sessionId: input.sessionId, env });
  await gov.recordReleaseEvent({
    conversationId, chainId: app.chain_id ?? (await getChainId(conversationId, session.orgId, env)),
    eventType: 'APP_SUBMITTED', riskTier: profile.risk_tier, qhubAppId: app.qhub_app_id,
    payload: {
      release_candidate_id: rcId, release_candidate_hash, qhub_app_version: up.rc.qhub_app_version,
      canonical_file_manifest_hash: partial.canonical_file_manifest_hash, file_count: partial.file_count,
      policy_profile_hash: profile.policy_profile_hash, enforcement_plan_hash: plan.enforcement_plan_hash,
      target_environment: partial.target_environment, required_attestation_purposes: requirements.map((r) => r.purpose),
      status: 'FROZEN',
    },
  });

  return {
    ok: true, release_candidate_id: rcId, release_candidate_hash, qhub_app_version: up.rc.qhub_app_version,
    required_attestations: requirements.map((r) => ({ purpose: r.purpose, roles: r.roles, min_signers: r.min_signers, distinct_signers: r.distinct_signers })),
    status: 'FROZEN',
  };
}
...
export async function signAttestation(input: SignInput): Promise<SignResult> {
  ...
  if (!rc) return { ok: false, reason: 'RELEASE_NOT_FOUND' };
  if (rc.org_id !== session.orgId) return { ok: false, reason: 'TENANT_MISMATCH' };
  if (!['FROZEN', 'AWAITING_ATTESTATION', 'APPROVED'].includes(rc.status)) return { ok: false, reason: 'NOT_FROZEN' };

  const profile = await getPolicyProfile(rc.qhub_app_id, env);
  const stored = await getActivePlan(rc.qhub_app_id, session.orgId, env);
  if (!profile || !stored) return { ok: false, reason: 'POLICY_STALE' };
  ...
  if (!req) return { ok: false, reason: 'SIGNER_NOT_AUTHORIZED' };

  // Server-authoritative signer authority: the session role must be permitted.
  if (!req.roles.includes(session.role)) return { ok: false, reason: 'SIGNER_NOT_AUTHORIZED' };
  ...
  const statementHash = sha256(canonicalStatementString({
    version: ATTESTATION_STATEMENT_VERSION, purpose: input.attestation_purpose, statement,
    release_candidate_hash: rc.release_candidate_hash, target_environment: rc.target_environment, scope,
  }));
  ...
  if (!ins.ok) return { ok: false, reason: 'DUPLICATE' };

  await store.setReleaseStatus(rc.release_candidate_id, session.orgId, 'AWAITING_ATTESTATION', env);

  const gov = createGovernanceService({ userId: session.userId, orgId: session.orgId, sessionId: input.sessionId, env });
  await gov.recordReleaseEvent({
    conversationId, chainId: null, eventType: 'ATTESTATION_SIGNED', riskTier: rc.risk_tier, qhubAppId: rc.qhub_app_id,
    payload: {
      attestation_id: att.attestation_id, release_candidate_id: rc.release_candidate_id, release_candidate_hash: rc.release_candidate_hash,
      qhub_app_version: rc.qhub_app_version, attestation_purpose: att.attestation_purpose, attestation_scope: scope,
      signer_role: att.signer_role, authority_reference: att.authority_source, target_environment: att.target_environment,
      policy_profile_hash: att.policy_profile_hash, enforcement_plan_hash: att.enforcement_plan_hash,
      attestation_statement_version: att.attestation_statement_version, attestation_statement_hash: att.attestation_statement_hash,
...
export async function evaluateReleaseForDeployment(input: EvalInput): Promise<EvalResult> {
  ...
  const reject = (codes: ReleaseReasonCode[], hash: string | null = null): EvalResult => ({
    ok: true, decision: 'REJECT', reason_codes: codes, satisfied_requirements: [], missing_requirements: [], decision_id: null, release_candidate_hash: hash,
  });
  ...
  const rc = await store.getReleaseCandidate(input.release_candidate_id, session.orgId, env);
  if (!rc) return reject(['RELEASE_NOT_FOUND']);
  if (rc.org_id !== session.orgId) return reject(['TENANT_MISMATCH']);
  if (rc.status === 'SUPERSEDED' || rc.status === 'REJECTED') return reject(['NOT_FROZEN'], rc.release_candidate_hash);
  if (!['FROZEN', 'AWAITING_ATTESTATION', 'APPROVED'].includes(rc.status)) return reject(['NOT_FROZEN'], rc.release_candidate_hash);
  if (rc.target_environment !== input.target_environment) return reject(['ENVIRONMENT_MISMATCH'], rc.release_candidate_hash);
  ...
  if (!classification || classification.classification_version !== rc.classification_version) return reject(['CLASSIFICATION_STALE'], rc.release_candidate_hash);
  if (!profile || profile.policy_profile_hash !== rc.policy_profile_hash) return reject(['POLICY_STALE'], rc.release_candidate_hash);
  if (!stored || stored.plan.enforcement_plan_hash !== rc.enforcement_plan_hash) return reject(['PLAN_STALE'], rc.release_candidate_hash);
  ...
  const evt = await gov.recordReleaseEvent({
    conversationId, chainId: null,
    eventType: decisionValue === 'APPROVE' ? 'DEPLOYMENT_APPROVED' : 'DEPLOYMENT_REJECTED',
    riskTier: rc.risk_tier, qhubAppId: rc.qhub_app_id,
    payload: {
      decision_id: decisionId, release_candidate_id: rc.release_candidate_id, release_candidate_hash: rc.release_candidate_hash,
      qhub_app_version: rc.qhub_app_version, decision: decisionValue, reason_codes: Array.from(reasons),
      satisfied_requirements: satisfied, missing_requirements: missing, target_environment: rc.target_environment,
      policy_profile_hash: rc.policy_profile_hash, enforcement_plan_hash: rc.enforcement_plan_hash,
    },
  });
  ...
  if (decisionValue === 'APPROVE' && !evt.ok) {
    ...
    await store.recordDeploymentDecision(
      { decision_id: decisionId, org_id: session.orgId, qhub_app_id: rc.qhub_app_id, release_candidate_id: rc.release_candidate_id, release_candidate_hash: rc.release_candidate_hash, decision: 'REJECT', reason_codes: ['DECISION_RECORD_FAILED'], satisfied_requirements: satisfied, missing_requirements: missing, target_environment: rc.target_environment, decided_by: session.userId },
      env,
    );

    return { ok: true, decision: 'REJECT', reason_codes: ['DECISION_RECORD_FAILED'], satisfied_requirements: satisfied, missing_requirements: missing, decision_id: null, release_candidate_hash: rc.release_candidate_hash };
  }

  await store.recordDeploymentDecision(
    {
      decision_id: decisionId, org_id: session.orgId, qhub_app_id: rc.qhub_app_id,
      release_candidate_id: rc.release_candidate_id, release_candidate_hash: rc.release_candidate_hash,
      decision: decisionValue, reason_codes: Array.from(reasons), satisfied_requirements: satisfied,
      missing_requirements: missing, target_environment: rc.target_environment, decided_by: session.userId,
    },
    env,
  );
  await store.setReleaseStatus(rc.release_candidate_id, session.orgId, decisionValue === 'APPROVE' ? 'APPROVED' : 'REJECTED', env);

  return {
    ok: true, decision: decisionValue, reason_codes: Array.from(reasons), satisfied_requirements: satisfied,
    missing_requirements: missing, decision_id: decisionId, release_candidate_hash: rc.release_candidate_hash,
  };
}
