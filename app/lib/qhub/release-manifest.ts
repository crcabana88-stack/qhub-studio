...
export function canonicalReleaseCandidateString(
  rc: Omit<ReleaseCandidate, 'release_candidate_id' | 'created_at' | 'created_by' | 'release_candidate_hash' | 'status' | 'supersedes_release_candidate_id'>,
): string {
  const canonical = {
 ...
 * distinct signers.
 */
export function deriveAttestationRequirements(
  ...
  if (att.has('OWNER_ATTESTATION')) {
    reqs.push({ requirement_id: 'ATT-OWNER', purpose: 'BUSINESS_OWNER', roles: ['owner', 'admin'], min_signers: 1, distinct_signers: false });
  }
  ...
  if (att.has('CHANGE_ATTESTATION')) {
    reqs.push({ requirement_id: 'ATT-CHANGE', purpose: 'TECHNOLOGY', roles: ['owner', 'admin', 'technology'], min_signers: 1, distinct_signers: false });
  }
...
export function canonicalReceiptString(
  r: Omit<ReleaseAssuranceReceipt, 'receipt_hash' | 'generated_at'>,
): string {
  return JSON.stringify({
    ...
    completed_attestations: r.completed_attestations.map((c) => `${c.purpose}|${c.signer_role}|${c.attestation_id}`).sort(),
