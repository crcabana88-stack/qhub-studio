# QHUB R15.5/R15.6 — Runtime Verifier Patch Runbook
...
R15.6 additionally closed four Codex-reproduced P1s: (1) `ALTER FUNCTION … IMMUTABLE STRICT
...
`13` and `14` authorize exactly **two** explained states — the documented live-era state or the
...
**13 (read-only)** verifies the complete start-state contract above and reports every check
...
**14 (one transaction, once)** replaces only the verifier. Gate 1 — **before** `CREATE OR
...
- `13` returns `UNEXPECTED_RUNTIME_VERIFIER_STOP` — an unexplained body digest, a missing owner
