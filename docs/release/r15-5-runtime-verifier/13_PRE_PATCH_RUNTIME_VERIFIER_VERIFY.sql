-- ============================================================================
-- QHUB R15.5 — 13 PRE-PATCH RUNTIME VERIFIER VERIFY (READ-ONLY)
--
-- Authorizes 14_LIVE_RUNTIME_VERIFIER_TRIGGER_ACL_PATCH.sql ONLY if the live
-- verifier is in a fully explained state:
--   * present at its exact zero-argument signature, no overload
--   * SECURITY DEFINER, exact fixed search_path
--   * exact direct ACL (service_role EXECUTE without grant option; PUBLIC/anon/
--     authenticated denied; no unexpected direct grantee or grant option)
...
--     forensics) OR already the reviewed R15.5 body (idempotent re-run).
...
SELECT
  ...
  coalesce((SELECT md5(v.prosrc) IN ('83c8cd60a96e44e6cb8d66db93daf403', 'f3c181abf13b54087eaf802ce11a29a4') FROM vobj v), FALSE)
                                                                                 AS is_reviewed_r15_5_body,
...
--     explained state (documented live start, or already R15.5), and no
...
vident AS (
  SELECT
    ...
    coalesce((SELECT v.proowner = (SELECT c.relowner FROM pg_class c
               WHERE c.oid = to_regclass('public.qhub_manual_review_requests')) FROM vobj v), FALSE)
                                                                                  AS owner_ok,
    coalesce((SELECT md5(v.prosrc) IN ('a35d8320d4a9804725a95f76534fe5a2', '83c8cd60a96e44e6cb8d66db93daf403', 'f3c181abf13b54087eaf802ce11a29a4') FROM vobj v), FALSE)
                                                                                  AS body_explained
),
vdirect AS (
  SELECT
    coalesce((SELECT count(*) = 1 FROM vobj v, aclexplode(v.proacl) ae
               WHERE ae.privilege_type = 'EXECUTE'
                 ...
                 AND NOT ae.is_grantable), FALSE)                                 AS service_role_execute,
    ...
    coalesce((SELECT NOT EXISTS (SELECT 1 FROM vobj v, aclexplode(v.proacl) ae
               WHERE ae.is_grantable AND ae.grantee <> v.proowner)), FALSE)       AS no_unexpected_grant_option
),
veff AS (
  SELECT
    (count(*) > 0
...
SELECT
  ...
  vdirect.service_role_execute, vdirect.no_unexpected_grantee, vdirect.no_unexpected_grant_option,
  ...
  CASE
    WHEN vident.verifier_present AND vident.zero_arg AND vident.single_fn
     ...
     AND vdirect.service_role_execute AND vdirect.no_unexpected_grantee
     AND vdirect.no_unexpected_grant_option
