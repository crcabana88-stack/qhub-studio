-- ============================================================================
-- QHUB R15.6 — 22 PATCH PROTECTED FUNCTION RESTORATION
...
-- at the reviewed commit â€” copied, never retyped.
--
...
--
-- WHY REVOKING IS SAFE â€” verified, not assumed. PostgreSQL checks EXECUTE on a trigger
...
--
-- qhub_decide_review's ACL is not mutated and must remain exactly the reviewed set
-- both before and after.
--
-- Run ONCE, as one transaction, only after the committed R15.6 restoration PRE returns
-- SAFE_TO_APPLY_PROTECTED_FUNCTION_RESTORATION. The exact live-start gates intentionally
-- reject a second run after success.
--
-- TRANSFER SAFELY OR NOT AT ALL:
--   Get-Content -Raw -Encoding UTF8 <file> | Set-Clipboard
-- Never PowerShell 5.1 Get-Content without -Encoding UTF8 â€” that is the defect this
-- package repairs, and it would silently reintroduce it.
-- ============================================================================

BEGIN;

-- Gate 0: the exact R15.6 verifier/report/trigger authority observed by Diagnostic 20.
DO $r15_6_patch_authority_pre$
DECLARE
...
BEGIN
  ...
  IF v_verifier IS NULL
     OR (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
          WHERE n.nspname='public' AND p.proname='qhub_verify_commercial_schema')<>1
     OR NOT EXISTS (
       SELECT 1 FROM pg_proc p JOIN pg_language l ON l.oid=p.prolang
        WHERE p.oid=v_verifier AND pg_get_function_identity_arguments(p.oid)=''
    ...
    v_failed := v_failed || 'verifier_contract';
  ELSE
  ...
  IF to_regclass('public.qhub_manual_review_requests') IS NULL
     ...
     OR EXISTS (
       SELECT 1 FROM unnest(ARRAY[
         'public.qhub_manual_review_requests','public.qhub_acknowledgments',
         'public.qhub_usage_ledger','public.qhub_entitlement_audit'
       ]) x(name)
    ...
    LOOP
      IF (SELECT count(*) FROM pg_trigger tg
           WHERE tg.tgname='trg_'||t||'_immutable'
             ...
             AND tg.tgenabled='O' AND NOT tg.tgisinternal AND tg.tgconstraint=0)<>1 THEN
        v_failed := v_failed || ('trigger_contract:'||t);
      END IF;
    END LOOP;
    IF (SELECT count(*) FROM pg_trigger tg WHERE tg.tgfoid=v_row AND NOT tg.tgisinternal)<>3 THEN
      v_failed := v_failed || 'trigger_binding_cardinality';
    END IF;
  END IF;

  IF cardinality(v_failed)>0 THEN
    RAISE EXCEPTION 'R15_6_PROTECTED_FUNCTION_PATCH_STOP:%', array_to_string(v_failed,',');
  END IF;
END;
$r15_6_patch_authority_pre$;


-- ---------------------------------------------------------------------------
-- GATE 1 â€” FAIL-CLOSED PRECONDITION (runs BEFORE any change).
...
-- ---------------------------------------------------------------------------
-- VERBATIM REVIEWED BODY 1 of 2 â€” public.qhub_decide_review
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- VERBATIM REVIEWED BODY 2 of 2 â€” public.qhub_row_immutable
-- ---------------------------------------------------------------------------
...
-- qhub_decide_review owner and ACL are intentionally untouched.

-- ---------------------------------------------------------------------------
-- R15.4 â€” NORMALIZE THE TRIGGER HELPER ACL to the reviewed owner-only contract.
...
-- ---------------------------------------------------------------------------
-- GATE 2 â€” FAIL-CLOSED CERTIFICATION BEFORE COMMIT.
-- Re-asserts the COMPLETE final contract: both reviewed bodies, every semantic and
-- callable attribute, owner/security/search_path, and BOTH final ACL sets â€” the
  ...
  LOOP
    ...
    IF r.sig = 'public.qhub_decide_review(uuid,text,boolean,text,text,text)' THEN
      IF NOT (coalesce((SELECT count(*) FROM aclexplode(p.proacl)) = 2, FALSE)
        ...
        RAISE EXCEPTION 'unexpected_function_acl_state: % direct ACL is not the exact reviewed set (live=%). Reviewed: qhub_decide_review: exactly 2 row(s) â€” owner, service_role (EXECUTE, granted by owner, not grantable).', r.sig, coalesce(p.proacl::text,'(NULL)'); END IF;
    ELSE
      IF NOT (coalesce((SELECT count(*) FROM aclexplode(p.proacl)) = 1, FALSE)
        ...
        RAISE EXCEPTION 'unexpected_function_acl_state: % direct ACL is not the expected reviewed target set (live=%). Expected: qhub_row_immutable: exactly 1 row(s) â€” owner (EXECUTE, granted by owner, not grantable).', r.sig, coalesce(p.proacl::text,'(NULL)'); END IF;
    END IF;
...
-- Gate 3: every final contract condition, including product readiness, before COMMIT.
DO $r15_6_patch_final$
DECLARE
  v_decide oid := to_regprocedure('public.qhub_decide_review(uuid,text,boolean,text,text,text)');
  v_row oid := to_regprocedure('public.qhub_row_immutable()');
  v_verifier oid := to_regprocedure('public.qhub_verify_commercial_schema()');
  v_report jsonb;
  v_failed text[] := ARRAY[]::text[];
  t text;
BEGIN
  IF v_decide IS NULL OR v_row IS NULL OR v_verifier IS NULL THEN
    v_failed := v_failed || 'final_identity';
  END IF;

  IF NOT coalesce((SELECT md5(p.prosrc) IN ('7e678f1e4bba0c540507cfe3743fbe54',
                                            'dac8abcd56d7fc804baac660059c14bf')
                    FROM pg_proc p WHERE p.oid=v_decide),FALSE) THEN
    v_failed := v_failed || 'final_decide_body';
  END IF;
  IF NOT coalesce((SELECT md5(p.prosrc) IN ('41ae59dde9a471b580d28e2cb45984f5',
                                            '4936e3f58627dde5abc10d2b0ecf5b4f')
                    FROM pg_proc p WHERE p.oid=v_row),FALSE) THEN
    v_failed := v_failed || 'final_row_body';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p WHERE p.oid=v_decide
      AND (SELECT count(*) FROM aclexplode(p.proacl))=2
      AND EXISTS (SELECT 1 FROM aclexplode(p.proacl) ae
                   WHERE ae.grantee=p.proowner AND ae.grantor=p.proowner
                     AND ae.privilege_type='EXECUTE' AND NOT ae.is_grantable)
    ...
    v_failed := v_failed || 'final_decide_acl_changed';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p WHERE p.oid=v_row
      AND (SELECT count(*) FROM aclexplode(p.proacl))=1
      AND EXISTS (SELECT 1 FROM aclexplode(p.proacl) ae
                   WHERE ae.grantee=p.proowner AND ae.grantor=p.proowner
                     AND ae.privilege_type='EXECUTE' AND NOT ae.is_grantable)
      AND NOT EXISTS (SELECT 1 FROM aclexplode(p.proacl) ae
                       WHERE ae.grantee<>p.proowner OR ae.grantor<>p.proowner
                          OR ae.privilege_type<>'EXECUTE' OR ae.is_grantable)
  ) THEN
    v_failed := v_failed || 'final_row_acl';
  END IF;
  ...
  LOOP
    IF (SELECT count(*) FROM pg_trigger tg
         WHERE tg.tgname='trg_'||t||'_immutable'
           ...
           AND tg.tgfoid=v_row AND tg.tgtype=27
           AND tg.tgenabled='O' AND NOT tg.tgisinternal AND tg.tgconstraint=0)<>1 THEN
      v_failed := v_failed || ('final_trigger:'||t);
    END IF;
  END LOOP;
  IF (SELECT count(*) FROM pg_trigger tg WHERE tg.tgfoid=v_row AND NOT tg.tgisinternal)<>3 THEN
    v_failed := v_failed || 'final_trigger_binding_cardinality';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    v_failed := v_failed || 'final_verifier_contract';
  ELSE
    v_report := public.qhub_verify_commercial_schema();
    IF v_report IS DISTINCT FROM jsonb_build_object(
         'ready',TRUE,
         'expected_version','2026-07-30.commercial-launch-r8',
         'failed','[]'::jsonb
       ) THEN
      v_failed := v_failed || 'final_product_report';
    END IF;
  END IF;

  IF cardinality(v_failed)>0 THEN
    RAISE EXCEPTION 'R15_6_PROTECTED_FUNCTION_PATCH_FINAL_STOP:%', array_to_string(v_failed,',');
  END IF;
END;
$r15_6_patch_final$;
